...
public class MetricsSyncService {
    ...
    public int syncAllMetrics() {
        ...
        for (InventoryMetrics stat : realTimeStats) {
            // PARTIAL Sync for Benchmark Test SKUs
            // These SKUs are manually seeded for the Supervisor Presentation.
            // We only update CURRENT STOCK to reflect user restock actions,
            ...
            if (existing != null) {
                ...
                if (!isBenchmark) {
                    // Only update stats for REAL products, keep benchmark stats frozen

...
public class TempDbController {
    ...
    @GetMapping("/api/temp/seed-db")
    public String seedDb() {
        try {
            jdbcTemplate.execute("SET FOREIGN_KEY_CHECKS = 0;");
            jdbcTemplate.execute("DELETE FROM inventory_metrics WHERE sku_id IN ('VG-OAT-001', 'VG-STW-002', 'VG-WAG-003', 'VG-TRK-004', 'VG-MNC-005');");
            jdbcTemplate.execute("DELETE FROM spoilage_log WHERE product_id IN (SELECT id FROM product WHERE sku IN ('VG-OAT-001', 'VG-STW-002', 'VG-WAG-003', 'VG-TRK-004', 'VG-MNC-005'));");
            jdbcTemplate.execute("DELETE FROM order_items WHERE product_id IN (SELECT id FROM product WHERE sku IN ('VG-OAT-001', 'VG-STW-002', 'VG-WAG-003', 'VG-TRK-004', 'VG-MNC-005'));");
            jdbcTemplate.execute("DELETE FROM product WHERE sku IN ('VG-OAT-001', 'VG-STW-002', 'VG-WAG-003', 'VG-TRK-004', 'VG-MNC-005');");
            ...
            return "SUCCESS - Database Seeded";
        } catch (Exception e) {
            e.printStackTrace();
